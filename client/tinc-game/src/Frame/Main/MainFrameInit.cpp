    ...
    if (getNetworks.success) {
        ...
        for (size_t networkIndex = 0; networkIndex < getNetworks.returnBody.size(); networkIndex++)
        {
            ...
            perNetworkData->ipTextCtrl = std::shared_ptr<wxTextCtrl>(new wxTextCtrl(rootPanel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_READONLY), rsx::wxTextCtrlDeleter);
            {
                auto minSize = wxDefaultSize;
                minSize.x = 10;
                perNetworkData->ipTextCtrl->SetMinSize(minSize);
            }
...
void MainFrame::Init_CreateControls()
{
...

    {
        ...
        networksAdvancedMenu->Append(wxIdMenuNetworksAdvancedDelete, _("&Delete"));
        Bind(wxEVT_MENU, &MainFrame::OnMenuNetworksAdvancedDelete, this, wxIdMenuNetworksAdvancedDelete);
        networksMenu->Append(wxID_ANY, _("&Advanced"), networksAdvancedMenu);

        menuBar->Append(networksMenu, _("&Networks"));
    }

    {
        toolsMenu = new wxMenu();
        toolsMenu->Append(wxIdMenuToolsManageTap, _("&Manage virtual network adapter"));
        Bind(wxEVT_MENU, &MainFrame::OnMenuToolsManageTap, this, wxIdMenuToolsManageTap);
        toolsMenu->Append(wxIdMenuToolsSettings, _("&Settings"));
        Bind(wxEVT_MENU, &MainFrame::OnMenuToolsSettings, this, wxIdMenuToolsSettings);

        toolsMenu->AppendSeparator();

        toolsAdvancedMenu = new wxMenu();
        toolsAdvancedMenu->Append(wxIdMenuToolsAdvancedOptimizeMtu, _("&Optimize MTU"));
        Bind(wxEVT_MENU, &MainFrame::OnMenuToolsAdvancedOptimizeMtu, this, wxIdMenuToolsAdvancedOptimizeMtu);
        toolsAdvancedMenu->Append(wxIdMenuToolsAdvancedTroubleshoot, _("&Troubleshoot"));
        Bind(wxEVT_MENU, &MainFrame::OnMenuToolsAdvancedTroubleshoot, this, wxIdMenuToolsAdvancedTroubleshoot);
        toolsMenu->Append(wxID_ANY, _("&Advanced"), toolsAdvancedMenu);
...
void MainFrame::Init_Layout()
{
...

    {
        ...
        networkControlNavigateSizer = new wxBoxSizer(wxHORIZONTAL);
        networkControlRootSizer->Add(networkControlNavigateSizer, 1);
        networkControlNavigateSizer->Add(recentActiveConnectButton.get(), 1, wxLEFT, ls::SpaceToFrameBorder);
        networkControlNavigateSizer->Add(recentActiveDisconnectButton.get(), 1, wxLEFT, ls::SpaceBetweenControl);
        ls::AddFixedSpacer(wxLEFT, 70, networkControlNavigateSizer);

        networkControlIpSizer = new wxBoxSizer(wxHORIZONTAL);
        networkControlRootSizer->Add(networkControlIpSizer, 1);
        ls::AddFixedSpacer(wxLEFT, ls::BoxSizerVhhOffset, networkControlIpSizer);
        networkControlIpSizer->Add(ipStaticText, 0, wxALIGN_CENTER_VERTICAL);
        ...
        auto what = networkControlIpSizer->GetMinSize();
